...
class World extends Container {
    ...
    // (WIP)
    // creates the path for the flies to follow
    createPath(x, y, length = 30, path = []) {
        ...
        let direction;
        // console.log(x, y);
        if (maxY < y && minY > y) direction = this.directions[0];
        else if (maxX > x && minX < x) direction = this.directions[3];
        else return path;

        // clones the tile and adds a direction component, then adds it to the path
        const newTile = tile.clone().add(direction);
        path.push(tile);
        return this.createPath(newTile.x, newTile.y, --length, path);
    }
